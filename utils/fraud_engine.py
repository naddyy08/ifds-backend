...
def check_large_quantity(transaction):
    ...
    # Only check stock_out and waste
    ...
    # Flag if more than 50% removed at once
...
def check_excessive_waste(transaction):
    ...
    # Flag if more than 30% reported as waste
...
def check_after_hours(transaction):
    """
    Flag transactions outside normal working hours
    Normal hours: 6 AM to 11 PM
    """
    
    transaction_hour = transaction.timestamp.hour
    
    # After hours: before 6 AM or after 11 PM
    if transaction_hour < 6 or transaction_hour >= 23:
        ...
        alert = FraudAlert(
            ...
            description=(
                ...
                f'Time: {transaction.timestamp.strftime("%H:%M")} '
                f'(Normal hours: 06:00 - 23:00). '
...
def check_rapid_removals(transaction):
    ...
    # Check transactions in last 1 hour for same item
    ...
    # Flag if more than 3 removals in last hour
...
def check_sudden_stock_drop(transaction):
    ...
    # Check if stock dropped critically low
    if item.reorder_level > 0:
        critical_level = item.reorder_level * 0.2
        
        # Flag if new quantity is below critical level
